...
func (c *UnitedDeploymentController) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	if found {
		newPolicy, err := genTemplate(deploy, clusterNames)
		if err != nil {
			klog.ErrorS(err, "gen template fail")
...
func genTemplate(deploy *v1.Deployment, clusterNames sets.String) (*policyv1alpha1.PropagationPolicy, error) {
	var labelSelector *metav1.LabelSelector
	var clusterNameStrs []string

	if deploy.Annotations != nil {
		if val, ok := deploy.Annotations["cluster-label-selectors"]; ok {
			if tmp, err := metav1.ParseToLabelSelector(val); err != nil {
				return nil, err
			} else {
				labelSelector = tmp
			}
		...
		if val, ok := deploy.Annotations["cluster-names"]; ok {
			clusterNameStrs = strings.Split(val, ",")
		}
	} else {
		clusterNameStrs = clusterNames.List()
	}
